...
then
	...
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$apache2_config/httpd.conf $apache2_dir
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$apache2_config/extra/httpd-vhosts.conf $apache2_dir/extra
	echo_ok "Fetching PHP Configurations from $sftp_host"
	# PHP 55
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/55/php.ini $php55_dir
	mkdir $php55_dir/conf.d
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/55/conf.d/ext-xdebug.ini $php55_dir/conf.d
	# PHP 56
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/56/php.ini $php56_dir
	mkdir $php56_dir/conf.d
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/56/conf.d/ext-xdebug.ini $php56_dir/conf.d
	# PHP 70
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/70/php.ini $php70_dir
	mkdir $php70_dir/conf.d
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/70/conf.d/ext-xdebug.ini $php70_dir/conf.d
	# PHP 71
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/71/php.ini $php71_dir
	mkdir $php71_dir/conf.d
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/71/conf.d/ext-xdebug.ini $php71_dir/conf.d
	# PHP 72
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/72/php.ini $php72_dir
	mkdir $php72_dir/conf.d
	sshpass -p $sftp_password sftp -oPort=$sftp_port $sftp_user@$sftp_host:$php_config/72/conf.d/ext-xdebug.ini $php72_dir/conf.d
	# RESTART APACHE
	apachectl -k restart
fi
